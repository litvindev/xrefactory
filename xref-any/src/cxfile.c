...
#define GET_VERSION_STRING(ttt) {\
	sprintf(ttt," file format: Xrefactory %s ",XREF_FILE_VERSION_NUMBER);\
}
...
static void cxrfVersionCheck(	int size,
	...
	if (strcmp(versionString, thisVersionString)) {
		writeCxFileCompatibilityError("The Tag file was not generated by the current version of Xrefactory, recreate it");
	}
	*ccc = cc; *ffin = fin;
}
