...
static void olGetAvailableRefactorings() {
	...
	if (! s_opt.xref2) {
		fprintf(ccOut,"* refactoring list not available in Xrefactory-I");
		return;
	}
...
static int tpCheckUnicityOfSymbol(char *fieldOrMethod) {
	...
	if (ss->next != NULL) {
		error(ERR_ST,"Ambiguous symbol. You are probably trying to refactor a constructor or a native method. Xrefactory does not know to handle those cases, sorry.");
		return(0);
	}
	return(1);
}
...
void mainAnswerEditAction() {
	...
	case OLO_CHECK_VERSION:
		...
		if (strcmp(s_opt.checkVersion, XREF_VERSION_NUMBER)!=0) {
			ppcGenRecord(PPC_VERSION_MISMATCH, XREF_VERSION_NUMBER, "\n");
		}
		break;
