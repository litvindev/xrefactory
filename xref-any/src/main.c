...
//&	fprintf(stdout,"\t-keep_old        - keep also all old references from 'refs' file\n");
	fprintf(stdout,"\t-no_stdop        - don't read the '~/.xrefrc' option file \n");
...
static void aboutMessage() {
	...
	sprintf(ttt,"This is Xrefactory version %s (%s).\n", XREF_VERSION_NUMBER, __DATE__);
...
/*fprintf(dumpOut,"try to open %s\n",ttt);*/
...
#ifdef __WIN32__	/*SBD*/
					"options to xref task. Please, use '%HOME%\\_xrefrc' file"
#else				/*SBD*/
#ifdef __OS2__		/*SBD*/
					"options to xref task. Please, use '%HOME%\\.xrefrc' file"
#else				/*SBD*/
					"options to xref task. Please, use '~/.xrefrc' file"
#endif				/*SBD*/
#endif				/*SBD*/
					);
			error(ERR_ST, tmpBuff);
	...
	if (s_opt.project==NULL) {
		...
		// I should check here whether the project still exists in the .xrefrc file
...
static void writeOptionsFileMessage( char *file, 
	...
	//&} else if (s_opt.xref2) {
		//& sprintf(tmpBuff,"Xrefactory project: %s", outSect);
		...
		if (s_opt.xref2) {
			...
			fprintf(dumpOut,"[Xref] active project: '%s'\n", outSect);
			fflush(dumpOut);
		}
	}
}
...
void getAndProcessXrefrcOptions(char *dffname, char *dffsect,char *project) {
	...
	if (*dffname != 0 && s_opt.stdopFlag==0 && s_opt.no_stdop==0) {
		...
		processOptions(dfargc, dfargv, INFILES_DISABLED); /* .xrefrc opts*/
	}
}
...
//&fprintf(dumpOut,"checking newcp==%s\n",s_opt.classpath);
	if (	*firstPassing 
		...
		/* piped options (no include or define options)
		   ...
		   must be set after .xrefrc file, but s_cachedOptions can't contain
           piped options, !!! berk.
		*/
		{ 
...
//&fprintf(dumpOut,"here we are '%s'\n",tt);fflush(dumpOut);
	...
	if (dffname[0]!=0) {
		...
		// disable error reporting on xref task on this pre-reading of .xrefrc
		if (s_opt.taskRegime==RegimeEditServer) {
...
static int mainSymbolCanBeIdentifiedByPosition(int fnum) {
	...
	// the .xrefrc file. so for the moment this will not work.
    // which problem ??????
